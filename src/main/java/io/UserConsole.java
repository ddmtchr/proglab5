...
public class UserConsole {
    private static final InputReceiver ir = new InputReceiver(new Scanner(System.in));
    private static final CommandExecutor commandExecutor = new CommandExecutor(ir);
     ...
     * @param ir InputReceiver for use to parse the string
     * @return Array that separately contains command[0] and arguments[1]
     */
    private static String[] parseCommand(InputReceiver ir) {
        String input = ir.getScanner().nextLine().trim();
        String cmd, args;
        if (input.contains(" ")) {
    ...
    public static void interact() {
        while (true) {
            try {
                String[] command = parseCommand(ir);
    ...
    public static int runScript(String scriptName) {
        ...
        try {
            ...
            Scanner latestScanner = ir.getScanner();
            ir.setFileReadMode(true);
            ir.setScanner(scriptScanner);
            do {
                String[] command = parseCommand(ir);
                if (command[0].equals("execute_script")) {
                    for (String script : scriptStack) {
                        if (command[1].equals(script)) {
                            ir.setFileReadMode(false);
                            ir.setScanner(latestScanner);
