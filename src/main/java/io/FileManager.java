        ...
        try {
            ...
            for (LabWork lw: CollectionManager.getCollection()) {
                ...
                if (lw.getCoordinates().getY() == null ||
                        lw.getCoordinates().getX() <= -934 || lw.getCoordinates().getY() > 946) throw new IncorrectFieldInputException();
                if (lw.getCreationDate() == null) throw new IncorrectFieldInputException();
                if (lw.getMinimalPoint() <= 0) throw new IncorrectFieldInputException();
                if (lw.getAveragePoint() <= 0) throw new IncorrectFieldInputException();
